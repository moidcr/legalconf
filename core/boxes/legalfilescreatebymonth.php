 ...
 */
class legalfilescreatebymonth extends ModeleBoxes
{
	/**
	 * @var string Alphanumeric ID. Populated by the constructor.
	 */
	public $boxcode = "legalcongbox1";
	...
	public function __construct(DoliDB $db, $param = '')
	{
		...
		$this->boxlabel = $langs->transnoentitiesnoconv("BoxLegalFilesCreateByMonth");
	...
	public function loadBox($max = 5)
	{
		...
		$text = $langs->trans("BoxLegalFilesCreateByMonth", $max);
		...
		if ($user->rights->commande->lire || 1==1)
		{
			...
			if ($shownb)
			{
				$data1 = $stats->getNbByMonthWithPrevYearLegal($endyear, $startyear, (GETPOST('action', 'aZ09') == $refreshaction ?-1 : (3600 * 24)), ($WIDTH < 300 ? 2 : 0), $startmonth, "ATC");

				$filenamenb = $dir."/".$prefix."filesnbinyear-".$endyear.".png";
				// default value for customer mode
				$fileurlnb = DOL_URL_ROOT.'/viewimage.php?modulepart=legalcongstats&amp;file=filesnbinyear-'.$endyear.'.png';
				...
				if (!$mesg)
				{
					...
					$px1->SetYLabel($langs->trans("NumberOfFiles"));
					$px1->SetShading(3);
					$px1->SetHorizTickIncrement(1);
					$px1->SetCssPrefix("cssboxes");
					$px1->mode = 'depth';
					$px1->SetTitle($langs->trans("NumberOfFilesByMonth"));
			...
			if ($showtot)
			{
				...
				$filenamenb = $dir."/".$prefix."filesamountinyear-".$endyear.".png";
				// default value for customer mode
				$fileurlnb = DOL_URL_ROOT.'/viewimage.php?modulepart=legalcongstats&amp;file=filesamountinyear-'.$endyear.'.png';
				...
				if (!$mesg)
				{
					...
					$px2->SetYLabel($langs->trans("AmountOfFilessHT"));
					$px2->SetShading(3);
					$px2->SetHorizTickIncrement(1);
					$px2->SetCssPrefix("cssboxes");
					$px2->mode = 'depth';
					$px2->SetTitle($langs->trans("AmountOfFilesByMonthHT"));
