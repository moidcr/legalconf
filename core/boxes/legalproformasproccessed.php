 ...
 */
class legalproformasproccessed extends ModeleBoxes
{
	/**
	 * @var string Alphanumeric ID. Populated by the constructor.
	 */
	public $boxcode = "legalcongbox4";
	...
	public function __construct(DoliDB $db, $param = '')
	{
		...
		$this->boxlabel = $langs->transnoentitiesnoconv("Top10ProformasProccessed");
	...
	public function loadBox($max = 5)
	{
		...
		//=====================================||||||Proformas procesadas||||||=====================
		
		
		$this->max = 0;//customer required all registers
        $this->info_box_head = array('text' => $langs->trans("Top10ProformasProccessed"));
		...
		if ($user->rights->birthdaysoc->myobject->read || 1==1)
		{
			...
			$sql = "select `soc`.`soc_id` AS `soc_id`, `soc`.`soc_name` AS `soc_name`, `soc`.`modified_on` ";
			
			$sql .= " from `bf_society` soc";
			$sql .= " WHERE `soc`.`soc_status` = 'Inactiva'";
			$sql .= " ORDER BY `soc`.`modified_on` DESC limit 0";
			...
			if ($result)
			{
				...
				while ($line < $num)
				{
                    ...
                    $this->info_box_contents[$line][] = array(
                        'td' => '',
                        //'text' => '<a href="'.DOL_URL_ROOT.'/legal/app/index.php/admin/files/society_pty/edit/'.$objp->soc_id.'"><span class="fas fa-folder infobox-adherent inline-block" style=""></span> '.$objp->soc_name.'</a>',
                        'text' => '<a href="#"><span class="fas fa-folder infobox-adherent inline-block" style=""></span> '.$objp->soc_name.'</a>',
                        'asis' => 1,
                    );
