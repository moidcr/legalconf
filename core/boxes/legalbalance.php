...
/**
 * \file    legalcong/core/boxes/legalbalance.php
 * \ingroup legalcong
 ...
 */
class legalbalance extends ModeleBoxes
{
	/**
	 * @var string Alphanumeric ID. Populated by the constructor.
	 */
	public $boxcode = "legalcongbox6";
	...
	public function __construct(DoliDB $db, $param = '')
	{
		...
		$this->boxlabel = $langs->transnoentitiesnoconv("BoxLegalBalance");
	...
	public function loadBox($max = 5)
	{
		...
		$text = $langs->trans("BoxLegalBalance", $max);
		...
		if ($user->rights->commande->lire || 1==1)
		{
			...
			if ($shownb)
			{
				$data1 = $stats->getBalanceByMonthWithPrevYearLegal($endyear, $startyear, (GETPOST('action', 'aZ09') == $refreshaction ?-1 : (3600 * 24)), ($WIDTH < 300 ? 2 : 0), $startmonth, "ATC");

				$filenamenb = $dir."/".$prefix."balancenbinyear-".$endyear.".png";
				// default value for customer mode
				$fileurlnb = DOL_URL_ROOT.'/viewimage.php?modulepart=legalcongstats&amp;file=balancesnbinyear-'.$endyear.'.png';
				...
				if (!$mesg)
				{
					...
					$px1->SetYLabel($langs->trans("NumberOfBalance"));
					$px1->SetShading(3);
					...
					$px1->mode = 'depth';
					$px1->SetTitle($langs->trans("BalanceByMonth"));
			...
			if ($showtot)
			{
				...
				$filenamenb = $dir."/".$prefix."balanceamountinyear-".$endyear.".png";
				// default value for customer mode
				$fileurlnb = DOL_URL_ROOT.'/viewimage.php?modulepart=legalcongstats&amp;file=balanceamountinyear-'.$endyear.'.png';
				...
				if (!$mesg)
				{
					...
					$px2->SetYLabel($langs->trans("AmountOfBalanceHT"));
					$px2->SetShading(3);
					$px2->SetHorizTickIncrement(1);
					$px2->SetCssPrefix("cssboxes");
					$px2->mode = 'depth';
					$px2->SetTitle($langs->trans("AmountOfBalanceByMonthHT"));
