...
/**
 * \file    test/phpunit/LegalCongFunctionalTest.php
 * \ingroup legalcong
 ...
 *
 * @package Testlegalcong
 */
class LegalCongFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modLegalCong')->click();
		} else {
			// Disable the module
			$this->byHref('modLegalCong')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/legalcong/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('legalcong/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/legalcong/admin/about.php');
		$this->authenticate();
		return $this->assertContains('legalcong/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('legalcongwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modLegalCong_LegalCongTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modLegalCong_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
