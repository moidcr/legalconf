...
/**
 * \file    legalcong/admin/setup.php
 * \ingroup legalcong
 * \brief   LegalCong setup page.
 */
...
require_once '../lib/legalcong.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "legalcong@legalcong"));
...
$arrayofparameters = array(
	'LEGALCONG_MYPARAM1'=>array('css'=>'minwidth200', 'enabled'=>1),
	'LEGALCONG_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/legalcong/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
elseif ($action == 'setmod') {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'LEGALCONG_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'LEGALCONG_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$page_name = "LegalCongSetup";
llxHeader('', $langs->trans($page_name));

// Subheader
$linkback = '<a href="'.($backtopage ? $backtopage : DOL_URL_ROOT.'/admin/modules.php?restore_lastsearch_values=1').'">'.$langs->trans("BackToModuleList").'</a>';

print load_fiche_titre($langs->trans($page_name), $linkback, 'object_legalcong@legalcong');

// Configuration header
$head = legalcongAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', '', -1, "legalcong@legalcong");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("LegalCongSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'LEGALCONG_'.strtoupper($myTmpObjectKey).'_ADDON';
