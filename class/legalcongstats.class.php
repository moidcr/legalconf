...
/**
 *       \file       htdocs/commande/class/legalcongstats.class.php
 *       \ingroup    commandes
 ...
 */
class LegalCongStats extends Stats
{
	/**
	...
	public function getNbByMonth($year, $format = 0, $type)
	{
		global $user;

		$sql = "SELECT date_format(c.created_on,'%m') as dm, COUNT(*) as nb";
		$sql .= " FROM bf_society as c";
		$sql .= " WHERE c.created_on BETWEEN '".$this->db->idate(dol_get_first_day($year))."' AND '".$this->db->idate(dol_get_last_day($year))."'";
		$sql .= " AND ".$this->where;
		$sql .= " GROUP BY dm";
		$sql .= $this->db->order('dm', 'DESC');
	 ...
	 */
	public function getNbByMonthWithPrevYearLegal($endyear, $startyear, $cachedelay = 0, $format = 0, $startmonth = 1, $type = '')
	{
		...
		if ($foundintocache)    // Cache file found and is not too old
		{
			...
			while ($year <= $endyear)
			{
				$datay[$year] = $this->getNbByMonth($year, $format, $type);
				$year++;
			}
	...
	public function getCasesByMonth($year, $format = 0, $type)
	{
		global $user;
	 ...
	 * @return 	array					Array of values
	 */
	public function getCasesByMonthWithPrevYearLegal($endyear, $startyear, $cachedelay = 0, $format = 0, $startmonth = 1, $type = '')
	{
		global $conf, $user, $langs;
		...
		if ($foundintocache)    // Cache file found and is not too old
		{
			...
			while ($year <= $endyear)
			{
				$datay[$year] = $this->getCasesByMonth($year, $format, $type);
				$year++;
			}
	...
	public function getBalanceByMonth($year, $format = 0, $type)
	{
		global $user;
	 ...
	 * @return 	array					Array of values
	 */
	public function getBalanceByMonthWithPrevYearLegal($endyear, $startyear, $cachedelay = 0, $format = 0, $startmonth = 1, $type = '')
	{
		global $conf, $user, $langs;
		...
		if ($foundintocache)    // Cache file found and is not too old
		{
			...
			while ($year <= $endyear)
			{
				$datay[$year] = $this->getBalanceByMonth($year, $format, $type);
				$year++;
			}
